...
from workflow.train_text_sql.utils_ds_ref import (GB, add_model_hooks, cache_bytes,
...
def get_ds_model(
    ...
    if getattr(config, 'torch_dtype', None) is None:
        dtype = torch.float16   # 默认使用半精度进行推理
    else:
    ...
    ''' 
        什么是磁盘卸载？
        简单来说，磁盘卸载是指 DeepSpeed 能够将模型训练或推理过程中产生的大量
        模型状态（包括模型参数、梯度和优化器状态）**从 GPU 内存和 CPU 内存进一步
        卸载到更慢但容量更大的 NVMe 固态硬盘 (SSD) 上
    '''
    if disk_offload:
    ...
    # model.eval() 作用于原始 PyTorch 模型，确保其进入评估模式（如关闭 Dropout、固定 BatchNorm 统计量）
    model = model.eval()


    ds_engine = deepspeed.initialize(model=model, config_params=ds_config)[0]
    '''
        1.DeepSpeed 的 initialize() 会重新包装模型结构（如分片参数、注入量化模块），
         可能重置模型状态。因此需对封装后的模型 ds_engine.module 再次调用 eval()
        2.若仅在原始模型调用 eval()，DeepSpeed 封装时可能因内部重构丢失状态，
          导致推理时启用 Dropout（输出随机）
        3.若仅在封装后调用，原始模型可能残留训练状态（如 BatchNorm 更新移动平均），
          干扰 DeepSpeed 的权重加载逻辑
    '''
...
def run_generation(
    ...
    if dummy:
        ...
        if not os.path.exists(filename):
            ...
            '''
                init_empty_weights():可以在不占用 GPU/CPU 显存的情况下，初始化任意规模的模型骨架
            '''
